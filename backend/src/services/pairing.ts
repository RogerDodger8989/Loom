...
class PairingService {
  ...
  private activeCodes = new Map<string, PendingPairing>();
  
  // Keyed by deviceId
  private activeDevices = new Map<string, string>(); // deviceId -> code
   ...
   */
  public requestPairing(deviceId: string): { code: string; deviceId: string; expiresAt: number } {
   ...
   */
  public confirmPairing(code: string, userId: string, deviceName: string = 'Unknown Device'): boolean {
   ...
   */
  public checkStatus(deviceId: string): { paired: boolean; userId?: string } {
    ...
    try {
      const stmt = db.prepare('SELECT user_id FROM paired_devices WHERE device_id = ?');
      const row = stmt.all(deviceId)[0] as { user_id: string } | undefined;
      if (row) {
