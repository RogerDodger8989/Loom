...
export class TMDBService {
      ...
      if (response.data && response.data.results && response.data.results.length > 0) {
        ...
        try {
          ...
          if (movieData.images && movieData.images.logos && movieData.images.logos.length > 0) {
            const preferredLogo = movieData.images.logos.find((l: any) => l.iso_639_1 === 'sv' || l.iso_639_1 === 'en') || movieData.images.logos[0];
            if (preferredLogo) {
          ...
          if (!movieData.trailer_url && prefLang !== 'en-US') {
            try {
              const videoRes = await axios.get(`${TMDB_BASE_URL}/movie/${match.id}/videos`, {
  ...
  public async fetchMovieById(id: string): Promise<any> {
    ...
    try {
      ...
      if (!movieData.trailer_url && prefLang !== 'en-US') {
        try {
          const videoRes = await axios.get(`${TMDB_BASE_URL}/movie/${id}/videos`, {
            params: { api_key: apiKey, language: 'en-US' }
          });
          if (videoRes.data && videoRes.data.results && videoRes.data.results.length > 0) {
            const enTrailer = videoRes.data.results.find((v: any) => v.site === 'YouTube' && v.type === 'Trailer' && v.official)
   ...
   */
  public async fetchShowById(id: string): Promise<any> {
    const apiKey = this.getApiKey();
    if (!apiKey) return null;
    ...
    try {
      const detailResponse = await axios.get(`${TMDB_BASE_URL}/tv/${id}`, {
        params: {
          api_key: apiKey,
          language: prefLang,
          append_to_response: 'credits,watch/providers,videos,keywords,similar,external_ids,images',
      ...
      if (showData.images && showData.images.logos && showData.images.logos.length > 0) {
        const preferredLogo = showData.images.logos.find((l: any) => l.iso_639_1 === 'sv' || l.iso_639_1 === 'en') || showData.images.logos[0];
        if (preferredLogo) {
      ...
      if (!showData.trailer_url && prefLang !== 'en-US') {
        try {
          const videoRes = await axios.get(`${TMDB_BASE_URL}/tv/${id}/videos`, {
            params: { api_key: apiKey, language: 'en-US' }
          });
          if (videoRes.data && videoRes.data.results && videoRes.data.results.length > 0) {
            const enTrailer = videoRes.data.results.find((v: any) => v.site === 'YouTube' && v.type === 'Trailer' && v.official)
   ...
   */
  public getImageUrl(path: string | null, size: 'w185' | 'w342' | 'w500' | 'original' = 'original'): string | null {
    if (!path) return null;
