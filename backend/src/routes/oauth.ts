...
export default async function oauthRoutes(fastify: FastifyInstance) {
  ...
  const renderResponsePage = (title: string, message: string, isError: boolean) => {
  ...
  fastify.get('/api/oauth/trakt/authorize', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!clientId) {
      ...
      return reply.code(400).send(renderResponsePage(
        'Trakt-konfiguration saknas', 
        'Ange ditt Trakt API Client ID (Trakt API Key) i Loom-inställningarna först innan du försöker ansluta.',
        true
      ));
    }
  ...
  fastify.get('/api/oauth/trakt/callback', async (request: FastifyRequest<{ Querystring: { code?: string } }>, reply: FastifyReply) => {
    ...
    if (!clientId || !clientSecret) {
      ...
      return reply.code(400).send(renderResponsePage(
        'Konfigurationsfel', 
        'Trakt Client ID och Client Secret måste båda vara ifyllda i Loom inställningar för att slutföra kopplingen.',
        true
      ));
    }
  ...
  fastify.get('/api/oauth/simkl/authorize', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!clientId) {
      reply.type('text/html');
      return reply.code(400).send(renderResponsePage(
        'Simkl-konfiguration saknas', 
        'Ange ditt Simkl Client ID i Loom-inställningarna först innan du försöker ansluta.',
        true
      ));
    }
  ...
  fastify.get('/api/oauth/simkl/callback', async (request: FastifyRequest<{ Querystring: { code?: string } }>, reply: FastifyReply) => {
    ...
    if (!clientId || !clientSecret) {
      ...
      return reply.code(400).send(renderResponsePage(
        'Konfigurationsfel', 
        'Simkl Client ID och Client Secret måste båda vara ifyllda i Loom inställningar för att slutföra kopplingen.',
        true
      ));
    }
