...
async function libraryRoutes(fastify) {
            ...
            const withCounts = paths.map(p => {
                ...
                try {
                    if (p.type === 'Movie') {
                        ...
                        const row = database_1.default.prepare("SELECT COUNT(*) as cnt FROM episodes WHERE file_path LIKE ? AND (deleted_at IS NULL OR deleted_at = '')").get(p.path + '%');
                        count = row?.cnt ?? 0;
                    }
    ...
    fastify.get('/api/library/recent-episode-groups', async (request, reply) => {
        try {
          ...
          SELECT
            ...
            MAX(COALESCE(e.added_at, mi.added_at)) AS group_added_at
          FROM episodes e
